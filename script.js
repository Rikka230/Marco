...
/*
  MARCO PATCH v0.4.0
  Type : polish visuel global
  Cible : sync page accent + classe body
*/
(() => {
  const PAGE_ACCENTS = {
    home: '#86f5a8',
  ...
  function syncVisualPolish() {
    const active = detectVisualPage();
    const accent = PAGE_ACCENTS[active] || PAGE_ACCENTS.home;

    document.body.classList.add('marco-visual-polish');
    document.body.dataset.marcoPage = active;

    document.documentElement.style.setProperty('--marco-page-accent', accent);
    document.documentElement.style.setProperty('--marco-nav-accent', accent);

    const nav = document.querySelector('[data-chapter-nav]');
    if (nav) {
      nav.style.setProperty('--marco-page-accent', accent);
      nav.style.setProperty('--marco-nav-accent', accent);
    }
  }

  function requestSyncVisualPolish() {
    if (frame) return;
  ...
  document.addEventListener('DOMContentLoaded', requestSyncVisualPolish);
  window.addEventListener('pageshow', requestSyncVisualPolish);
  window.addEventListener('popstate', requestSyncVisualPolish);

  const observer = new MutationObserver(requestSyncVisualPolish);

  function bootVisualPolish() {
    requestSyncVisualPolish();

    const app = document.querySelector('#app');
    if (app && !app.dataset.marcoVisualPolishObserved) {
      app.dataset.marcoVisualPolishObserved = 'true';
      observer.observe(app, {
        childList: true,
        subtree: true,
        attributes: true,
        attributeFilter: ['class', 'data-page', 'data-theme', 'src']
      });
    }
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', bootVisualPolish);
  } else {
    bootVisualPolish();
  }
})();
